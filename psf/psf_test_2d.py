...
def load_image(image_path, mode='slice'):
    ...
    Args:
        ...
        mode: 'slice' for 3D stack, 'sum' for projection

    Returns:
        Tuple of (image data, is_3d flag)
    """
    img = np.array(tifffile.imread(image_path)).astype(np.float32)

    if len(img.shape) == 3:
        if mode == 'sum':
            img = np.sum(img, axis=0) / 65535.0
...
def main():
    parser = argparse.ArgumentParser(description='Test PSF blur on 2D images')
    parser.add_argument('--image', type=str, required=True, help='Input image filename')
    ...
    parser.add_argument('--mode', type=str, default='slice', choices=['slice', 'sum'],
                       help='Processing mode for 3D images')
